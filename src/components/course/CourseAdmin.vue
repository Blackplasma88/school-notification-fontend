    ...
    getInstructorlist() {
      ...
      for (let i = 0; i < this.subject_list.length; i++) {
        if (this.subject_list[i].name === this.subject_name) {
          ...
          for (
            let j = 0;j < this.subject_list[i].instructor_id.length;j++
          ) {
            axios
              .get(
                "http://127.0.0.1:8080/profile/profile_id?profile_id="+this.subject_list[i].instructor_id[j]+"&role=teacher")
              .then((response) => {
